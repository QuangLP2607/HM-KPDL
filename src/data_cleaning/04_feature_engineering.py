...
def flatten_wiki_data(wiki_data: Dict) -> pd.DataFrame:
    ...
    for province, districts in wiki_data.items():
        for district, info in districts.items():
            ...
            # Tính toán các chỉ số phát triển
            commune_density = round(communes / area, 2) if area > 0 else 0
...
def merge_datasets(
    ...
    """
    Merge dữ liệu chính với dữ liệu wiki.
...
def main():
    # Cấu hình
    config = {
        'input': 'data/interim/03_processed_missing.csv',
    ...
    try:
        ...
        save_path = os.path.join(config['output_dir'], '04_feature_engineering.csv')
        df_merged, not_merged = merge_datasets(df, wiki_data, save_path)
        
        # Lưu báo cáo
        report_path = os.path.join(config['report_dir'], '04_merge_analysis.txt')
